...
function wizPoll()
        {
			window.open('', 'wizPoll_<?=$uid?>','width=<?=$popwidth?>,height=<?=$popheight?>,marginwidth=0,marginheight=0,resizable=no,scrollbars=auto'); 
        }
//-->		
</script>
...
if(!ereg("([^[:space:]]+)",$uid) || !$uid) {
	echo "<script>window.alert('잘못된 경로로 부터의 접금입니다.');history.go(-1);</script>";
}
else {
	$total = $dbcon->get_one("select count(*) from wizpoll where UID='$uid'");
	if($total == 0) {
		echo"생성되지 않은 코드입니다.";
		exit;
	}
...
## 투표 하기
if($mode == "" && !${"wizPollCookie".$uid}) {
?>
		<form action='./util/wizpoll/DEFAULT/result.php' method='post' onsubmit ='javascript:wizPoll()' target='wizPoll_<?=$uid?>'>
...
?>
          ...
          &nbsp;&nbsp;&nbsp;<img src="./util/wizpoll/DEFAULT/images/left_ic03.gif" onclick="javascript:window.open('./util/wizpoll/DEFAULT/result.php?uid=<?=$uid?>&mode=view','WizPOLLViewWindow_<?=$uid?>','width=<?=$popwidth?>,height=<?=$popheight?>,srollbar=auto,toolbar=no,statusbar=no');" style="cursor:pointer";></div></td>
                                      </tr></form>
