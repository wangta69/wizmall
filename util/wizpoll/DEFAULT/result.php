...
*** Updating List ***
2004. 06. 18 - globals_off 상황에서도 동작되게 설계
*/
...
<?
if($mode == "poll" && !${"wizPollCookie".$uid}) {
    ...
    if(!${"wizPollCookie".$uid}) {
		...
		// 항목 투표수 1 증가
		$Result = $dbcon->_query("UPDATE wizpoll SET Vote='$vote_ok' where UID='$uid'");
		
		// 투표 한사람에게 쿠키 생성(1일)
		//setcookie("wizPollCookie".$PID,"1",time()+86400);
	// 결과 보여주기
	LocationReplace("$PHP_SELF?uid=$uid&mode=view&window_open=$window_open&width=$width");
...
## 투표 결과
else if($mode == "view" || ${"wizPollCookie".$uid}) {
