...
function getBigPicture(no){
	wizwindow('./skinwiz/viewer/<?php echo $cfg["skin"]["ViewerSkin"]?>/picview.php?no='+no, 'BICIMAGEWINDOW','width=750,height=592,statusbar=no,scrollbars=no,toolbar=no,resizable=no')
}
//-->
</script>
...
<form name='view_form' id="view_form" action='./wizbag.php' method="post" onsubmit='return checkForm(this);'>
	...
	<input type="hidden" name="source_id" VALUE="<?php echo $view["source_id"]?>">
	<input type="hidden" name="sub_query" VALUE= "">
	<input type="hidden" name="op" VALUE= "<?php echo $op?>">
	<div class="col-lg-5">
		<a href="javascript:getBigPicture(<?php echo $view["source_id"]?>)"><img src="<?php echo $view["imgsrc"]?>" width="100%" class="img-thumbnail"/></a>
		<div class="agn_c">
			<a href="javascript:getBigPicture(<?php echo $view["source_id"]?>)"><img src="./skinwiz/viewer/<?php echo $cfg["skin"]["ViewerSkin"]?>/images/but_zoom.gif" width="83"></a>
		</div>
	</div>
	<div class="col-lg-7">
		<span class="b orange">
			...
			<?php echo $mall->ShowOptionIcon($cfg["skin"]["ShopIconSkin"], $view["Regoption"]);?>
		</span>
		<table class="table table-striped table-hover">
			...
			<tr>
				...
				<td> :
					<?php echo number_format($view["Price1"])?>
					원</td>
			</tr>
			<?endif;?>
			<?if($view["Brand"]):?>
			...
			<tr>
				...
				<td> :
					<?php echo number_format($view["Point"])?></td>
			</tr>
			<? endif;?>
			<?if($view["CompName"]):?>
			<tr>
...
?>
					<tr>
						<th class="agn_l"><?php echo $oname?></th>
						<td> :
<?php
	if($valuecnt == "1"){//옵션등록갯수가 하나이면 일반 텍스트 디스플레이
		...
		$checkstr = $oflag == "0"?" checkenable msg='".$oname."를 선택해 주세요'":"";
?>
							<select name="optionfield[<?php echo $ouid?>]" class="formline" <?php echo $checkstr?> oflag="<?php echo $oflag;?>" onchange="checkthis(this)">
								<option value=''>
								<?php echo $oname?>
								선택</option>
<?php
					...
					<tr>
						...
						<ul class="pd_cnt">
							...
							<li style="padding:0px;"><ul style="padding-left:5px;">
								<li><a href="javascript:num_plus(document.view_form);"><img src="./skinwiz/viewer/<?php echo $cfg["skin"]["ViewerSkin"]?>/images/num_plus.gif"></a></li>
								<li><a href="javascript:num_minus(document.view_form);"><img src="./skinwiz/viewer/<?php echo $cfg["skin"]["ViewerSkin"]?>/images/num_minus.gif"></a></li>
							</ul></li>
							<li>EA</li>
						</ul>
						</td>
...
-->
				...
				<div class="btn_box">
					...
					<span class="button bull"><a href="<?php echo $pre_dir?>">리스트</a></span>
					<?if ($cfg["member"]):?>
					<span class="button bull"><a href='./skinwiz/wizwish/index.php?uid=<?php echo $view["UID"];?>' onclick='return wishconfirm();'>상품찜</a></span>
					<? else:?>
...
?>
				<td><table>
						<tr>
							<td><a href="<?php echo $mall->pdviewlink($UID,$Category,$None)?>"><img src="<?php echo $img_path?>" <?php echo $View_Pic_Size?>></a></td>
						</tr>
						<tr>
							<td><?php echo $sublist["Name"]?>
								<br />
								<?php echo $sublist["Model"]?>
								<br />
								<?php echo number_format($sublist["Price"])?>
								원</td>
						</tr>
					</table></td>
<?php
...
<div class="panel">
	<div class="panel-heading">
		<h3 class="panel-title">상품평 보기 | 고객님에게 꼭 맞는 상품 구입을 위한 유용한 자료로 사용하세요! <span class="button bull"><a href="javascript:wizwindow('./skinwiz/viewer/<?php echo $cfg["skin"]["ViewerSkin"]?>/estimatepopup.php?query=<?php echo $query?>&code=<?php echo $code?>&no=<?php echo $no?>&GID=<?php echo $view["source_id"]?>','','width=554,height=450')">상품평쓰기</a></span></h3>
	</div>
	<div class="panel-body">
		<form name="estimat" action="<?php echo $PHP_SELF?>" onsubmit='return check_reple_Form();'>
	<input type="hidden" name="query" value="<?php echo $query?>">
	<input type="hidden" name="code" value="<?php echo $code?>">
	<input type="hidden" name="no" value="<?php echo $view["source_id"]?>">
	<input type="hidden" name="repleqry" value="insert">
	<input type="hidden" name="Name" value="<?php echo $cfg["member"]["mname"]?>">
...
<table class="table table-striped table-hover">
	<tr>
		...
		<td><?php echo $list["Name"]?>
			...
			&nbsp;&nbsp;&nbsp;<a href="javascript:reple_delete('<?php echo $list["UID"]?>');" >x</a>
			<?endif;?></td>
		<th>고객선호도 </th>
		<td><img src="./skinwiz/viewer/<?php echo $cfg["skin"]["ViewerSkin"]?>/images/star<?php echo $list["Grade"]?>.gif"></td>
	</tr>
	<tr>
		<td colspan="4"><?php echo $list["Subject"]?></td>
	</tr>
	<tr>
		<td colspan="4" style="word-break:break-all;"><?php echo $list["Contents"]?>
		</td>
	</tr>
</table>
<?
