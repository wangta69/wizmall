...
(function($) {
	$.fn.formvalidate = function() {
		...
		$(':input', this).each(function (index) {
			...
			input_tag = $(this).get(0).tagName.toLowerCase();
			if (typeof input_attr =="undefined") input_attr = input_tag.toLowerCase();
			if(input_attr == "input")input_attr = "text";
	...
	$.validate = {
		...
		text : function($f) { 
			...
			if(min_len && str_len < min_len){
				...
				if(group_email_class){//두개의 텍스트 필드 비교하기
					var tmp	= new Array();
