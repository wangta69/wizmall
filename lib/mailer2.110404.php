<?php
include "./mail/xml_parse.php";//이종간 테이블 연동을 위해 xml 이용
$xml_parser = new xml();
/************************************************/
/**                                            **/
...
//첨부파일을 배열에 저장해 둔다.
	...
	if ($userfile!="none" && $userfile) {
	  if(!move_uploaded_file($userfile, "../config/tmp_upload/$userfile_name")) {
...
if($soption == "testMail"){
	if(mail($testMailAddress, $subject, $body_txt, $add_header)){  
...
//주의 xml부분은 보완상으로 취약하므로 다른 처리가 필요하다.
	$data = "query=select&mailreject=$mailreject&startno=$startno&Roopcnt=$Roopcnt&soption=$soption&startseq=$startseq&stopseq=$stopseq&startdate=$startdate&stopdate=$stopdate&genderSelect=$genderSelect&gradeSelect=$gradeSelect&server=".$_SERVER['SERVER_NAME'];
	$data = $common->getencode($data);
	$file = $cfg["admin"]["MART_BASEDIR"]."/malladmin/mail/shopwizMemberInfo.php?data=".$data;
	...
	if(is_array($xml_parser->filename["TOTAL"])){
		foreach($xml_parser->filename["TOTAL"] as $key=>$value){
			$totalmail = $value;
		}
	}

	if(is_array($xml_parser->filename["EMAIL"])){
		foreach($xml_parser->filename["EMAIL"] as $key => $value){
			if(trim($value)){
				$mailto	= trim($value);
				$name	= trim($xml_parser->filename["NAME"][$key]);		
