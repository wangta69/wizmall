  ...
  <form NAME="board_search" action="<?=$PHP_SELF?>" mehtod="POST" onsubmit="return boardSearch(this)">
    ...
    <input type="hidden" name="cp" value="<?=$cp?>">    
        ...
        <!--<select name="SEARCHTITLE" checkenable msg="검색범위를 선택해주세요">-->
          ...
          <option value="SUBJECT"<? if($SEARCHTITLE == "SUBJECT" ) echo " selected";?>>제 목</option>
          <option value="CONTENTS"<? if($SEARCHTITLE == "CONTENTS" ) echo " selected";?>>내 용</option>
        </select></td>
...
?>
          <tr>
            <td height="26" align="center">공지</td>
            <td></td>
            <td align="left">&nbsp;<a href="<?=$PHP_SELF?>?getdata=<?=$getdata?>">
      <?=$list["print_subject"]?>
      </a></td>
            <td></td>
            <tdalign="center"><?=date("Y.m.d", $list["W_DATE"])?></td>
            <td></td>
...
?>
          <tr>
            <td height="26" align="center"><?=$board->ini_board_no;?></td>
            <td></td>
            <td align="left">&nbsp;<a href="<?=$PHP_SELF?>?getdata=<?=$getdata?>">
      <?=$list["print_subject"]?>
      </a></td>
